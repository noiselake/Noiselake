import { getScreen, div } from "../util/screens.js";
...
const intro = [
	"You are the Bro, dude!",
	"",
	"You threw a totally rad party last night and have a bad case of the munchies right now.",
	"",
	"Go check some of the boxes in your mom's basement for pizza.",
	"",
	"",
	"Oh and btw, watch out for Pedro, he's a real bum.",
	"",
	"Controls:",
	"Space - check box",
	"Arrow keys - move"
...
function addTemplate(container, id) {
...
async function brogue() {
	...
	return new Promise(async resolve => {
		// LOGO
		let logoScreen = getScreen("logo");
		addTemplate(logoScreen, "logo");
		let enter = div(logoScreen, "enter");
		pause(2);
		type("--- PRESS ENTER ---", { initialWait: 5 }, enter);

		await waitForKey();
		logoScreen.remove();

		// INTRO
		let introScreen = getScreen("intro");
		type(intro, {}, introScreen);
		await waitForKey();
		introScreen.remove();
